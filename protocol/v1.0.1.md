# TensorSwing Record Protocol, version 1.0.1
...
Version 1.0.1 makes three corrections. Per §14 they apply forward only:
`protocol/v1.0.0.md` is untouched, and any week whose manifest pins
1.0.0 remains governed by 1.0.0.

1. §10.1 no longer places a copy of the manifest inside the encrypted
   archive: the manifest pins `archive_sha256` over the published
   archive bytes, so an archive containing the final manifest bytes
   would require a SHA-256 fixed point and cannot be produced. The
   archive layout is restated without it.
2. §1.1 and §1.2 are reworded inside the §1.4 register ("sampled
   subset", "beacon-selected sample" replace phrasings that used a
   §1.4-banned word).
3. §8.4 pins the exact canonical shape of an abstain leaf, and §13.1
   step 7 scopes the numeric label-consistency and price-plausibility
   checks to prediction leaves, with an explicit abstain branch.
...
- `protocol_version` — the semver string of the governing protocol; for
  weeks governed by this document, exactly `"1.0.1"`.
...
any additional pipeline fields.
